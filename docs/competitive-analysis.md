...
## D. 本会话实现进度（2026-06-04）

本会话按「冲突感知波次」落地了 §C 中 **7 个缺口**（均带离线单测，tsc + 全套件全绿）：

| §C# | 缺口 | 实现要点 | commit 主题 |
|---|---|---|---|
| #2 | 13 大师结果缓存 | **磁盘**缓存（非内存——sidecar spawn-per-call 模型内存缓存无效）；key=symbol+lang+大师集+provider:model+量化定性信号+新闻；10min TTL+50 LRU。命中跳过 15 次 LLM | `feat(sidecar): 磁盘结果缓存` |
| #3 | 波动率→建议仓位上限% | `src/lib/positionSizing.ts` 波动率目标法纯函数（base 20%，夹 5%-25%）；RiskCard 渲染 + 非投资建议口径 | `feat(quant): 建议仓位上限` |
| #1 | valuation/risk 回灌复合分 | computeComposite 四维化：blend 估值方向（无估值时零排序扰动）+ risk 向中性收敛；breakdown 透明暴露 | `feat(quant): 复合分四维化` |
| #4 | 量化评分可下钻 | 基本面 scorer 产 CheckItem{actual,threshold,comparator,passed}，QuantScoreCard 展开渲染 ✓/✗ | `feat(quant): 量化下钻` |
| #5 | 大师分歧度可视化 | `computeMasterConsensus` 前端纯函数 + 三色多空占比条 | `feat(deep-analysis): 大师多空分布` |
| #8 | 波动率亏损区间 | ±1.96σ√t 折算「半年 95% 区间」，下行 -100% 夹紧 | `feat(quant): 波动率涨跌区间` |
| #10 | A 股资金流数据 | 东财 fflow/daykline（端点经 WebFetch 实测核实）主力/超大/大/中/小单，A 股 gate，FundFlowCard | `feat(quant): A股资金流卡` |

**延后清单（带明确解锁条件，非遗漏）：**

| §C# | 缺口 | 为何延后 | 解锁条件 |
|---|---|---|---|
| #6 / #7 | AI 价位 / 回测结果叠加 K 线 | 图表渲染**无法盲改验证**，且 ChartCanvas 有精细 ref 生命周期，盲改回归风险高 | 需在能跑应用、可视觉 QA 的会话做 |
| #9 | AI 回答 inline 溯源 | 价值取决于 LLM 返回**准确**来源；越界内的错误来源比无来源更糟（反幻觉功能的反作用） | 需真 LLM 在环验证来源准确性 + 跨 provider prompt 稳定性 |
| #13 | 大师信号回流动态权重 | 对已有命中率展示边际价值低，且有 false precision 风险 | 待有明确产品诉求再做 |
| #11 / #12 / #14 | 财报 RAG / 因子预计算 / NL 选股 | 缺基础设施：历史财务时序（当前仅当期快照）/ 全市场数据端点 | 先建 `financial_history` 表 + 全市场端点 |

## E. 一句话结论

StockAI 的护城河是**「下载即用 + 免费自带 key + 桌面本地隐私 + 13 大师中文叙事 + 多 provider + 三语」的产品化打磨**；本会话把「算了却没用」的资产接通并补齐 2026 三条入场券——**四维名副其实（#1）+ 抗幻觉可解释（#4/#5）+ 可执行风控（#3/#8）**，并补上 A 股门面第一块（#10 资金流）与漏钱点（#2 缓存）。剩余缺口要么需视觉/真 LLM 验证、要么缺数据基础设施，已列明解锁条件。
