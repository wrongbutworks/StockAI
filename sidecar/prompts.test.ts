...
describe('buildAnalysisPrompt', () => {
  const news = [
    createMockNews({
      ...
      source: 'Reuters',
      content:
        '苹果公司在秋季发布会上推出了全新 iPhone 系列产品，搭载最新的 A20 芯片，性能提升显著，发布会现场吸引了大量关注。',
    }),
...
describe('buildEnhancedPrompt', () => {
  ...
  const quant = createMockQuantBundle({
    ...
    fundamental: {
      signal: 'neutral',
      confidence: 55,
      details: { roe: 16.5, pe: 22, net_margin: 12 },
    },
...
describe('buildAnalysisPrompt with language', () => {
  const news = [
    createMockNews({
      ...
      source: 'Reuters',
      content:
        'Apple unveiled a new iPhone series at its annual fall event. The device features the latest chip.',
    }),
  ];
