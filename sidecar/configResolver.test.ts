...
describe("resolveConfig 角色分级（roleModels）", () => {
  ...
  const multiProviderConfig = {
    ...
    providerConfigs: {
      ...
      deepseek: { apiKey: "sk-deepseek", baseUrl: "https://api.deepseek.com", model: "deepseek-v4-pro" },
    },
  };
  ...
  test("各角色指向不同 provider 时取对应 key/baseUrl + role 的 model", () => {
    ...
    expect(cfg.roles.brain).toMatchObject({ provider: 'openai', apiKey: 'sk-openai', model: 'gpt-4o' });
    expect(cfg.roles.quick).toMatchObject({ provider: 'deepseek', apiKey: 'sk-deepseek', model: 'deepseek-chat' });
