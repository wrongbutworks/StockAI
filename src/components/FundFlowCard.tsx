...
const FundFlowCard: React.FC<FundFlowCardProps> = ({ fundFlow }) => {
  ...
  const color = (v: number) => (v > 0 ? 'text-emerald-400' : v < 0 ? 'text-rose-400' : 'text-gray-400');
  ...
  return (
    <div className="mb-6">
      <h2 className="text-gray-400 text-xs font-bold mb-4 uppercase tracking-widest">{t('fund_flow')}</h2>
      <div className="p-4 bg-white/5 rounded-2xl border border-white/5 space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span className={`text-sm font-bold ${color(fundFlow.mainNet)}`}>
            {sign(fundFlow.mainNet)}{yi(fundFlow.mainNet)} {t('unit_yi')}
          </span>
        </div>
        <div className="grid grid-cols-2 gap-x-3 gap-y-1">
          {orders.map(o => (
            <div key={o.label} className="flex items-center justify-between text-xs">
              ...
              <span className={color(o.value)}>{sign(o.value)}{yi(o.value)}</span>
            </div>
          ))}
        </div>
        <p className="text-[10px] text-gray-600 mt-1">{fundFlow.date} · {t('fund_flow_note')}</p>
      </div>
    </div>
  );
};
