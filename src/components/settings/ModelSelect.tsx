...
function formatListModelsError(err: unknown, t: TFn): string {
  if (err instanceof ServiceError) {
    switch (err.code) {
      case "ERR_LIST_MODELS_TIMEOUT": return t("err_list_models_timeout");
      case "ERR_LIST_MODELS_AUTH": return t("err_list_models_auth");
      case "ERR_LIST_MODELS_NETWORK": return t("err_list_models_network");
      case "ERR_LIST_MODELS_SERVER": return t("err_list_models_server", { message: err.message });
      case "ERR_LIST_MODELS_BAD_REQUEST": return t("err_list_models_bad_request", { message: err.message });
      default: return t("err_list_models_generic", { message: err.message });
    }
  }
  return t("err_list_models_generic", { message: err instanceof Error ? err.message : String(err) });
}
 ...
 */
export function ModelSelect({ provider, baseUrl, apiKey, value, onChange }: ModelSelectProps): React.ReactElement {
  const { t } = useLanguage();
